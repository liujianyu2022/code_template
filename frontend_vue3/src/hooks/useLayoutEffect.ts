...
const useLayoutEffect = (effect: Effect, deps?: any[]): void => {
    // 确保在组件上下文中调用
    if (!getCurrentInstance()) throw new Error('useLayoutEffect must be called within a component setup function');
    ...
    // 使用 onUpdated 而不是 watch 来确保在 DOM 更新后同步执行
    const stop = watch(
        getWatchSource(),
        () => {
            if (deps?.length === 0) {
            ...
            // 使用 queueJob 确保在下一个微任务队列中执行，但仍然在浏览器绘制前
            Promise.resolve().then(() => {
                applyEffect();
            });
        },
        { immediate: true, flush: 'post' }, // 使用 'post' 选项确保在 DOM 更新后执行
    );
